...
class FAABasedQueue<E> : Queue<E> {
    ...
    override fun enqueue(element: E) {
        /*while (true) {
            ...
            if (curTail.id < segmentNode.id) {
                tail.compareAndSet(curTail, segmentNode)
            }
            val segment = segmentNode.segment
            if (segment[i % SEGMENT_SIZE].compareAndSet(null, element)) return
        }*/
        while (true) {
            val curTail = tail.value
    ...
    override fun dequeue(): E? {
        /*while (true) {
            ...
            if (curHead.id < segmentNode.id) {
                head.compareAndSet(curHead, segmentNode)
            }
            val segment = segmentNode.segment
            if (segment[i % SEGMENT_SIZE].compareAndSet(null, POISONED)) {
                continue
            }
            return segment[i % SEGMENT_SIZE].value as E
        }*/
        while (true) {
            if (deqIdx.value >= enqIdx.value) return null
            val curHead = head.value
